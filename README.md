...
[![Build Status](https://travis-ci.org/mobxjs/mobx-preact-lite.svg?branch=master)](https://travis-ci.org/mobxjs/mobx-preact-lite)
[![npm](https://img.shields.io/npm/v/mobx-preact-lite.svg)](http://npm.im/mobx-preact-lite)
...
This package provides the bindings for MobX and Preact. See the [official documentation](http://mobxjs.github.io/mobx/intro/overview.html) for how to get started. Code and documentation originated from [mobx-preact-lite](https://github.com/mobxjs/mobx-preact-lite). The feature set of `mobx-preact` is a slightly slimmed down adaptation of `mobx-react`. The features available are reflected by this document. The features omitted are:
...
When using `mobx-preact-lite` you can define a new life cycle hook, `componentWillReact` (pun intended) that will be triggered when a component is scheduled to be re-rendered because data it observes has changed. This makes it easy to trace renders back to the action that caused the rendering.
